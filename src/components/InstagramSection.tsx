const COLLAGE = '/images/284CFAFD-7C99-440A-9E04-1C5A55683283.png'
const pos = (col: number, row: number) =>
  `${col * 25}% ${parseFloat((row * 33.333).toFixed(2))}%`

const galleryImages = [
  { pos: pos(1, 0), alt: 'Dalgalı kahve saç' },
  { pos: pos(0, 1), alt: 'Koyu bob kesim' },
  { pos: pos(2, 0), alt: 'Açık kahve dalgalı saç' },
  { pos: pos(1, 2), alt: 'Arka görünüm uzun saç' },
  { pos: pos(2, 3), alt: 'Kahverengi dalgalı portre' },
  { pos: pos(3, 3), alt: 'Gelin saç tasarımı' },
  { pos: pos(0, 3), alt: 'Salon iç mekan' },
  { pos: pos(4, 3), alt: 'Sarı topuz tasarımı' },
]

export default function InstagramSection() {
  return (
    <section
      id="galeri"
      className="py-24 relative overflow-hidden"
      style={{ background: '#FFF8F5' }}
    >
      <div className="max-w-7xl mx-auto px-6">
        {/* Header */}
        <div className="text-center mb-14">
          <p
            className="text-xs uppercase tracking-[0.25em] mb-4"
            style={{ fontFamily: 'Inter, sans-serif', color: '#B9816F' }}
          >
            Galeri
          </p>
          <h2
            className="text-4xl lg:text-5xl mb-2"
            style={{ fontFamily: 'Instrument Serif, serif', color: '#1F1A17' }}
          >
            @hulyahairbeauty
          </h2>
          <div
            className="mx-auto mb-5"
            style={{ height: '2px', width: '48px', background: '#B9816F', borderRadius: '1px' }}
          />
          <a
            href="https://instagram.com/hulyahairbeauty"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-full px-6 py-2.5 text-sm font-medium transition-all duration-300 hover:scale-105"
            style={{
              fontFamily: 'Inter, sans-serif',
              background: 'linear-gradient(135deg, #833ab4, #fd1d1d, #fcb045)',
              color: 'white',
              boxShadow: '0 4px 16px rgba(131,58,180,0.30)',
            }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
            </svg>
            Instagram&apos;da Takip Et
          </a>
        </div>

        {/* Masonry-style grid */}
        <div
          className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4"
          style={{ gridAutoRows: '160px' }}
        >
          {galleryImages.map((img, idx) => {
            const isLarge = idx === 0 || idx === 5
            return (
              <div
                key={idx}
                className="group relative rounded-2xl overflow-hidden cursor-pointer"
                style={{
                  gridRow: isLarge ? 'span 2' : 'span 1',
                  boxShadow: '0 4px 20px rgba(0,0,0,0.10)',
                }}
              >
                {/* Collage crop */}
                <div
                  aria-label={img.alt}
                  role="img"
                  className="absolute inset-0 transition-transform duration-500 group-hover:scale-110"
                  style={{
                    backgroundImage: `url('${COLLAGE}')`,
                    backgroundSize: '500% 400%',
                    backgroundPosition: img.pos,
                  }}
                />
                {/* Hover overlay */}
                <div
                  className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"
                  style={{ background: 'rgba(201,143,122,0.40)' }}
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="white">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                  </svg>
                </div>
              </div>
            )
          })}
        </div>
      </div>
    </section>
  )
}
